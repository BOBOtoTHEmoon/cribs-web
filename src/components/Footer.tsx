    ...
    <footer style={{ borderTop: "1px solid var(--card-border)" }} className="pt-14 pb-8">
      <div className="max-w-[1200px] mx-auto px-5 sm:px-10">
        ...
        <div className="md:hidden pb-8">
          ...
          {/* Get Started as 2x2 */}
          <div className="mt-7">
            <div className="text-[0.62rem] font-bold uppercase tracking-[2px] mb-4" style={{ color: "var(--dim)" }}>Get Started</div>
            <div className="grid grid-cols-2 gap-x-6 gap-y-3">
              {GET_STARTED.map((l) => (
                <Link key={l.label} href={l.href} className="block text-[0.82rem] transition-colors" style={{ color: "var(--text)" }} onMouseEnter={(e) => (e.currentTarget.style.color = "var(--cream)")} onMouseLeave={(e) => (e.currentTarget.style.color = "var(--text)")}>{l.label}</Link>
              ))}
            </div>
          </div>
        </div>
        ...
        {/* DESKTOP layout */}
        <div className="hidden md:grid md:grid-cols-[2fr_1fr_1fr_1fr] gap-6 pb-12">
          <Brand />
          <LinkCol title="Explore" links={EXPLORE} />
          <LinkCol title="Support" links={SUPPORT} />
